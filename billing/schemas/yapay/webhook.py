...
class OrderWebhookData(BaseModel):
    deliveryStatus: Optional[DeliveryStatus] = Field(None, description="Статусы доставки")
    orderId: str = Field(..., description="ID заказа, полученный в ответе /order/create")
    paymentStatus: Optional[PaymentStatus] = Field(None, description="Статус заказа")


class WebhookV1Request(BaseModel):
    event: Event
    eventTime: datetime = Field(..., description="время события в формате `RFC 3339`; `YYYY-MM-DDThh:mm:ssTZD`")
    merchantId: UUID
    operation: Optional[OperationWebhookData] = None
    order: Optional[OrderWebhookData] = Field(None, description="если event == ORDER_STATUS_UPDATED")
